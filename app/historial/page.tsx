  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100">
      ...
      <main className="container mx-auto px-4 py-8">
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-slate-900 mb-2">Historial de Compras</h1>
          <p className="text-slate-600">Gestiona y revisa todas las compras realizadas</p>
        </div>
        ...
        {filteredSales.length > 0 && (
          <div className="mt-6 bg-white rounded-lg shadow-sm border border-slate-200 p-6">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <div>
                <p className="text-sm text-slate-600 mb-1">Venta total</p>
                <p className="text-2xl font-bold text-slate-900">
                  {formatCurrency(filteredSales.reduce((sum, sale) => sum + sale.total, 0))}
                </p>
              </div>
              <div>
                <p className="text-sm text-slate-600 mb-1">Saldo restante</p>
                <p className="text-2xl font-bold text-blue-600">
                  {formatCurrency(filteredSales.reduce((sum, sale) => sum + sale.commission, 0))}
                </p>
              </div>
            </div>
          </div>
            ...
            <Table>
              <TableHeader>
                <TableRow className="bg-slate-50">
                  ...
                  <TableHead className="font-semibold">Fecha de la compra</TableHead>
                  <TableHead className="font-semibold">Estatus</TableHead>
                  <TableHead className="font-semibold">No. de celular / No. de servicio</TableHead>
                  <TableHead className="font-semibold">Nombre del servicio</TableHead>
                  <TableHead className="font-semibold text-right">Subtotal</TableHead>
                  <TableHead className="font-semibold text-right">Comisión</TableHead>
                ...
                {filteredSales.length === 0 ? (
                  ...
                  filteredSales.map((sale) => (
                    <TableRow key={sale.id} className="hover:bg-slate-50">
                      ...
                      <TableCell className="text-slate-600">{formatDate(sale.date)}</TableCell>
                      <TableCell>
                      ...
                      <TableCell className="text-slate-600">{sale.phone}</TableCell>
                      <TableCell className="text-slate-900 font-medium">{sale.company}</TableCell>
                      <TableCell className="text-right text-slate-900">{formatCurrency(sale.subtotal)}</TableCell>
